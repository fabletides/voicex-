/*
 * VoiceX - Система восстановления голоса с персонализацией через ML
 * Полная реализация для ESP32 с TensorFlow Lite
 * 
 * Компоненты:
 * - Захват сигнала с горлового микрофона
 * - Предобработка и извлечение признаков
 * - ML inference для персонализации голоса
 * - Воспроизведение через вибромоторы
...
bool load_tflite_model() {
    ...
    // Попытка загрузки из SPIFFS
    xSemaphoreTake(spiffs_mutex, portMAX_DELAY);
    
    uint8_t* model_data = nullptr;
    size_t model_size = 0;
    bool loaded_from_spiffs = false;
    
    if (SPIFFS.exists(MODEL_FILE_PATH)) {
        Serial.println("📁 Загрузка модели из SPIFFS...");
        
        File model_file = SPIFFS.open(MODEL_FILE_PATH, "r");
        if (model_file) {
            model_size = model_file.size();
            Serial.printf("📊 Размер модели в SPIFFS: %d байт\n", model_size);
            
            model_data = (uint8_t*)ps_malloc(model_size);
            if (model_data) {
                model_file.read(model_data, model_size);
                model_file.close();
                loaded_from_spiffs = true;
                Serial.println("✅ Модель загружена из SPIFFS");
            } else {
                Serial.println("❌ Не удалось выделить память для модели из SPIFFS");
                model_file.close();
            }
        }
    }
    ...
    // Если не удалось загрузить из SPIFFS, используем встроенную модель
    if (!loaded_from_spiffs) {
        Serial.println("📦 Использование встроенной модели...");
        
        #ifdef VOICEX_MODEL_DATA_H
        // Включение встроенной модели
        #include "voicex_model_data.h"
        
        if (validate_voicex_model()) {
            model_data = (uint8_t*)get_voicex_model_data();
            model_size = get_voicex_model_size();
            Serial.printf("📊 Размер встроенной модели: %d байт\n", model_size);
            Serial.println("✅ Встроенная модель валидна");
        } else {
            Serial.println("❌ Встроенная модель повреждена");
            return false;
        }
        #else
        Serial.println("❌ Встроенная модель не доступна");
        Serial.println("💡 Варианты решения:");
        Serial.println("   1. Загрузите model.tflite в SPIFFS");
        Serial.println("   2. Включите voicex_model_data.h в проект");
        Serial.println("   3. Используйте веб-интерфейс для загрузки модели");
        return false;
        #endif
    }
    ...
    if (tflite_model->version() != TFLITE_SCHEMA_VERSION) {
        ...
        if (!loaded_from_spiffs) {
            // Не освобождаем память встроенной модели
        } else {
            free(model_data);
        }
        return false;
    ...
    Serial.printf("   Источник: %s\n", loaded_from_spiffs ? "SPIFFS" : "встроенная");
    Serial.printf("   Размер: %d байт (%.2f KB)\n", model_size, model_size/1024.0f);
    ...
    Serial.printf("   Тип входа: %s\n", 
                  input_tensor->type == kTfLiteInt8 ? "INT8" : "FLOAT32");
    Serial.printf("   Тип выхода: %s\n", 
                  output_tensor->type == kTfLiteInt8 ? "INT8" : "FLOAT32");
    
    // Если загружено из SPIFFS, сохраняем указатель для освобождения позже
    if (loaded_from_spiffs) {
        // Можно сохранить указатель в глобальной переменной для последующего освобождения
        // Но для простоты оставляем как есть - память освободится при перезапуске
    }
...
// Веб-сервер для управления устройством
#include <WiFi.h>
#include <WebServer.h>
            ...
            fetch('/status')
                ...
                .then(data => {
                    document.getElementById('status').innerHTML = 
                        `Состояние: ${data.state}<br>
                         Модель: ${data.model_loaded ? 'Загружена' : 'Не загружена'}<br>
                         Память: ${data.free_heap} байт`;
...
void handle_status() {
    ...
    doc["state"] = get_state_name(current_state);
    doc["model_loaded"] = (interpreter != nullptr);
...
void handle_restart() {
    server.send(200, "application/json", "{\"message\": \"Перезапуск...\"}");
    delay(1000);
    ESP.restart();
}
...
void handle_test() {
    ...
    server.send(200, "application/json", "{\"message\": \"Тест выполнен\"}");
}

void handle_logs() {
    server.send(200, "text/plain", "VoiceX система работает нормально");
}